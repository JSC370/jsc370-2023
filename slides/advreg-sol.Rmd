...
```{r}
lmod<-lm(co2~dec_date,data=co2_s)
summary(lmod)

co2_s2<-co2[co2$year<2010,]

lmpredco2<-predict(lmod,co2_s2)
...
gmod<-gam(co2~s(dec_date, k=20, fx=TRUE, bs="cr"), data=co2_s)
...
```{r}
gmod2<-gam(co2~s(dec_date,bs="cr",k=20)+s(month, bs="cc",k=6), data=co2_s)
plot(gmod2)
summary(gmod2)

gpred<-predict.gam(gmod2,co2)
plot(gpred,type='l')
```
